...
public class AdminPermissionsTests
{
    ...
    [Fact]
    public void SAML設定は特権管理者だけが持つ()
    {
        foreach (var role in Enum.GetValues<AdminRole>())
        {
            Assert.Equal(
                role == AdminRole.Administrator,
                AdminPermissions.Of(role).Contains(AdminPermissions.SamlSettings));
        }
    }

    [Fact]
    public void Pleasanterのログイン設定は特権管理者だけが持つ()
    {
        foreach (var role in Enum.GetValues<AdminRole>())
        {
            Assert.Equal(
                role == AdminRole.Administrator,
                AdminPermissions.Of(role).Contains(AdminPermissions.PleasanterSsoSettings));
        }
    }
    ...
    public void メンテナンス操作は特権管理者だけが持つ()
    {
        foreach (var role in Enum.GetValues<AdminRole>())
        {
            Assert.Equal(
                role == AdminRole.Administrator,
                AdminPermissions.Of(role).Contains(AdminPermissions.MaintenanceManage));
        }
    }
