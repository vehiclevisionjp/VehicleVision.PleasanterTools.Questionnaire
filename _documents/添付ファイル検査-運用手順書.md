...
このうちサイズ・個数・許可拡張子の上限と、`QUESTIONNAIRE_VIRUSSCAN_TIMEOUTSECONDS`・
`QUESTIONNAIRE_VIRUSSCAN_DEFENDER_RESULTTIMEOUTSECONDS` は、管理画面の「設定」（アプリケーション設定）からも変えられる。
**外部設定で指定した項目はそちらが優先され、画面では固定表示になる**
（`Web/Services/AppSettingsProvider.cs` の定義一覧と読み込み順）。
有効化・方式・接続先・Defender の資格情報は画面に出さず、外部設定だけで与える。
...
**添付を受け付けない。** `503` を返し、サーバのログへエラーを出す（`Endpoints/FormEndpoints.cs`）。
**ClamAV を有効にしている場合は、添付の送信が無くても定期監視する。**
DB の起動完了後に固定テキストを検査し、確認の終了から 1 分後に次の確認を行う。
待ち時間の上限は通常の検査と同じ `QUESTIONNAIRE_VIRUSSCAN_TIMEOUTSECONDS` を使う。

- 初回から検査できない場合、または正常から異常に変わった場合は、
  「ClamAV の検査が利用できません」を管理画面のお知らせへ追加し、Error ログを出す。
- 異常の後に検査できた場合は「ClamAV の検査が復旧しました」を追加し、Information ログを出す。
  初回の正常確認は復旧として通知しない。
- 同じ状態が続く間は新しいお知らせを作らない。再発時は既存の同種の未読通知へ回数を加算する。
  **回数は状態変化の検知回数であり、拒否した添付の数ではない。**
- お知らせを保存できなかった場合はログに残し、次回に再試行する。
  その間に状態が変わった場合は最新の状態を優先する。

状態はアプリのインスタンスごとに記憶し、再起動でリセットする。
複数インスタンスでは同じ障害をそれぞれ検知するため、回数が増える場合がある。
お知らせは観測の履歴であり、全インスタンスの現在の正常性を保証するものではない。
監視の間隔より短い障害や定義ファイルの鮮度はこの監視だけでは分からない。
通常の添付検査と拒否時のログは継続する。Defender for Storage はこの定期監視の対象外。

実装: `Web/Services/Attachments/ClamAvHealthMonitor.cs`、
`ClamAvHealthMonitorHostedService.cs`、`Core/Notifications/AdminNotificationKind.cs`
（`src/VehicleVision.PleasanterTools.Questionnaire.*` 配下、2026-09-25）。
