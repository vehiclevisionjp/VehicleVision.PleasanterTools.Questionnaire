...
Azure App Service へ導入する場合も、A をサイドカーとして構成できる
**Linux Web App が推奨構成**である。Windows Web App は本アプリを動かせるが、
App Service のサイドカーを使えないため 3 章の Windows 向け選択肢が別途必要になる。
...
### Windows Web App でも動かす場合の選択肢

Windows Web App では App Service のサイドカーを構成できない。`clamd` はネットワーク越しに
利用できるため、`QUESTIONNAIRE_VIRUSSCAN_HOST` と
`QUESTIONNAIRE_VIRUSSCAN_PORT` で接続先を指定する。

|選択肢|内容|
|---|---|
|別の場所の `clamd` へ接続する|Container Apps、Azure Container Instances、自前の VM などで `clamd` を動かし、同じ仮想ネットワークに閉じて接続する。インターネットへ公開しないこと。常時稼働するため、別途費用がかかる|
|Microsoft Defender for Storage|利用できるが判定は非同期である。結果を待つ上限を超えたら回答ごと断るため、大きな添付では実質利用できない（4 章）|
|添付を使わない|検査なしで添付を受け付けない。未検査のバイナリを DB に載せない決まりを守ること|
...
ClamAV を常時稼働させられない導入先では、次のいずれかを選ぶ。

| 道 | 使える所 | 注意 |
| --- | --- | --- |
| **ClamAV サイドカー** | **Linux の App Service（既定）** | **App Service プランのメモリを消費する** |
| **別の場所の `clamd`** | どこでも | **本アプリと同じ仮想ネットワークに閉じる。** `clamd` の常時稼働費用がかかる |
| **Defender for Storage** | Azure | **判定が非同期。** 判定を待てるよう、回答の添付サイズ上限を下げる |
| **添付を使わない** | どこでも | **いちばん安全。** 設問に添付項目を置かない |
| **検査を切って受け付ける** | **閉じた環境のみ** | **運用者の選択として認める。** リスクを引き受ける必要がある |
...
管理者が登録する配布資産（Issue #266）は、回答者が送信する添付とは事情が違う。
配布前に待たせられるため、非同期の Defender for Storage でも成立する。
したがって、**回答の添付は `clamd`、配布資産は Defender for Storage**という組み合わせもあり得る。

ただし、現在の実装は回答の添付と配布資産で
`QUESTIONNAIRE_VIRUSSCAN_*` の有効・無効と方式を共有しており、
どちらか一方を選ぶ作りである。両者を別方式にするかは未確定であり、#295 の続きとして決める。
