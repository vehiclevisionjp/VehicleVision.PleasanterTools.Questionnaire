...
- **Windows へ配置する場合は AMSI という選択肢もある**（OS 同梱）。
  ⚠️ **「本アプリは Linux コンテナ前提」と書いてあったが、事実と違う。**
  [`導入-更新運用手順書.md`](導入-更新運用手順書.md) 4 章は
  **Windows Web App を推している**（Issue #293）。
  **AMSI は未実装で、実装するかも決まっていない。** 3 章の選択肢を参照すること
