...
```apache
# 末尾スラッシュ無しの /questionnaire を /questionnaire/ へ送る（無いと Pleasanter へ流れる）
# ProxyPass は RedirectMatch より先に効くので、この 1 本だけ proxy から外す
ProxyPassMatch "^/questionnaire$" "!"
RedirectMatch 301 "^/questionnaire$" "/questionnaire/"
...
`/questionnaire/` の行を `/` の行より先に書く（`ProxyPass` は書いた順に照合し、最初に一致したものが使われる）。
`X-Forwarded-For` は mod_proxy_http が既定で付ける。本アプリ側で `QUESTIONNAIRE_FORWARDED_NETWORKS` に
Apache のアドレス（同じ機械なら `127.0.0.1/32`）を登録する。

⚠️ **末尾スラッシュ無しの `/questionnaire` の扱いを忘れない。** `ProxyPassMatch ... "!"` と `RedirectMatch` が
無いと、`/questionnaire` は `ProxyPass "/"` に拾われて Pleasanter へ流れ、Pleasanter の「見つからない」画面になる。
**`RedirectMatch` だけを足しても効かない。** `ProxyPass` の照合が先に行われるため、`"!"` で proxy から外す
必要がある（2026-09-25 に httpd 2.4 で実測。優先順位そのものは mod_proxy の資料に書かれていない）。

### 3.3 Nginx

```nginx
# 末尾スラッシュ無しの /questionnaire を /questionnaire/ へ送る。
# 転送先を相対にして、前段のポートや https を落とさない
absolute_redirect off;
location = /questionnaire {
    return 301 /questionnaire/;
}
...
⚠️ **`absolute_redirect off;` を書く理由。** 末尾がスラッシュの `location` を `proxy_pass` で処理すると、
Nginx は末尾スラッシュ無しの要求（`/questionnaire`）へ自分で 301 を返す。既定（`absolute_redirect on`）では
転送先が絶対 URL になり、**前段のポートが落ちる**（2026-09-25 に Nginx 1.29 で `http://localhost:8091/questionnaire`
が `http://localhost/questionnaire/` へ送られることを実測）。前段で https を終端する構成では http へ落ちるおそれもある。
`off` にすると転送先が相対（`/questionnaire/`）になる。効くのは **Nginx 自身が返す転送だけ**で、
Pleasanter や本アプリが返す転送（`Location` ヘッダ）は変えない。例の断片は `server` ブロックの中に書く。
...
- [Apache HTTP Server: mod_proxy](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html)
  ...
  `"!"` による除外、書いた順に照合し最初に一致したものが使われること（参照 2026-09-25）
- [nginx: ngx_http_core_module — absolute_redirect / location](https://nginx.org/en/docs/http/ngx_http_core_module.html#absolute_redirect)
  — `off` で Nginx が返す転送が相対になる（1.11.8 以降）。末尾がスラッシュの `location` を `proxy_pass` で
  処理すると、末尾スラッシュ無しの要求へ 301 を返す（参照 2026-09-25）
...
| 構成 | 状況 |
| --- | --- |
| Kestrel を直接 `QUESTIONNAIRE_PATH_BASE=/questionnaire` で起動 | 確認済み（2026-09-24。`PathBaseTests.cs` と手元の起動） |
| ANCM が PathBase を渡す場合の二重回避 | 単体テストで確認（ANCM の振る舞いを真似たもの） |
| **Nginx**（3.3 の例） | **確認済み**（2026-09-25。下記） |
| **Apache**（3.2 の例） | **確認済み**（2026-09-25。下記） |
| IIS のサブアプリケーション（3.1） | 未検証 |
| Azure App Service の仮想アプリケーション（3.4） | 未検証 |
| Azure Front Door / Application Gateway（3.5） | 未検証 |
| AKS の Helm チャート（Application Routing Ingress） | 未検証（`helm template` の描画だけ確認。[`AKS-導入更新運用手順書.md`](AKS-導入更新運用手順書.md)） |

**Nginx・Apache の確認の仕方。** Docker で Nginx 1.29（`nginx:1.29-alpine`）と Apache httpd 2.4
（`httpd:2.4-alpine`）を立て、3.2・3.3 の例をそのまま写した（変えたのは宛先のホスト名だけ）。
後ろに Pleasanter 1.5.8.1 と、`QUESTIONNAIRE_PATH_BASE=/questionnaire` を設定した本アプリ（SQLite）を置いた。
次の結果は Nginx・Apache で同じだった。

| 手順 | 結果 |
| --- | --- |
| `/questionnaire/healthz`・`/questionnaire/admin` | 200（本アプリが応答） |
| `/users/login` | 200（Pleasanter が応答） |
| `/questionnaire`（末尾スラッシュ無し） | 301 で `/questionnaire/` へ（3.2・3.3 の転送の設定を足した後） |
| 最初の管理者を作ったときの cookie `q.admin` の Path | `/questionnaire` |
| Pleasanter のシングルサインオン | 往復できた（[Pleasanter-SSO-運用手順書](Pleasanter-SSO-運用手順書.md)） |

⚠️ **転送の設定を足す前は、末尾スラッシュ無しの `/questionnaire` で困った。** Nginx はポートを落とした
絶対 URL へ送り、Apache は Pleasanter の「見つからない」画面を返した。3.2・3.3 の例はこれを直した後のもの。

HTTPS の終端、SAML の往復、メールのリンクは、この確認には含めていない。
