...
未翻訳の言語を選んでも、画面自身の文言は日本語（`ja`）へ落ちるため、器だけを先に開けても
画面は壊れない。

| 本アプリのコード | 言語 | 画面カタログ |
|---|---|---|
| `en` | 英語 | 翻訳済み |
| `zh` | 中国語 | 未翻訳（`ja` へフォールバック） |
| `ja` | 日本語 | 翻訳済み（既定） |
| `de` | ドイツ語 | 未翻訳（`ja` へフォールバック） |
| `ko` | 韓国語 | 未翻訳（`ja` へフォールバック） |
| `es` | スペイン語 | 未翻訳（`ja` へフォールバック） |
| `vi` | ベトナム語 | 未翻訳（`ja` へフォールバック） |
...
| `Frontend/src/lib/i18n/messages.ts` | 回答画面のカタログ（**約 107 件**。未翻訳言語は `ja` へフォールバック） |
| `Frontend/src/admin/lib/i18n/messages.ts` | 管理画面のカタログ（**約 615 件**。未翻訳言語は `ja` へフォールバック） |
| `Core/Localization/SupportedLanguages.cs` | `All` へ言語コードを足す |

- **サーバ側（C#）は、言語コードを足すだけでビルドが通る。**
  カタログに無い言語は `ja` へ落ちる（1 章）
