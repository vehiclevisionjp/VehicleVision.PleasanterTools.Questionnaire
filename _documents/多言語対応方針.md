...
器として対応する言語は **7 つ**。7 言語とも画面の文言カタログを持つ（Issue #452〜#456 で訳した）。
訳の抜けた鍵があっても、画面自身の文言は英語（`en`）を経て日本語（`ja`）へ落ちるため、画面は壊れない。

| 本アプリのコード | 言語 | 画面カタログ |
|---|---|---|
| `en` | 英語 | 翻訳済み |
| `zh` | 中国語 | 翻訳済み（管理画面の一部は `en` へフォールバック。下記） |
| `ja` | 日本語 | 翻訳済み |
| `de` | ドイツ語 | 翻訳済み（管理画面の一部は `en` へフォールバック。下記） |
| `ko` | 韓国語 | 翻訳済み（管理画面の一部は `en` へフォールバック。下記） |
| `es` | スペイン語 | 翻訳済み（管理画面の一部は `en` へフォールバック。下記） |
| `vi` | ベトナム語 | 翻訳済み（管理画面の一部は `en` へフォールバック。下記） |

**鍵の数（2026-09-25、v0.7.0 時点で数えた）。** 回答画面は 7 言語とも 124 / 124 件。
管理画面は `ja`・`en` が 989 件、`zh`・`de`・`ko`・`es`・`vi` は 958 件で、
Issue #464 で足した Pleasanter のログイン設定の文言（`pleasanterSso.*` の 31 件）がまだ訳されておらず `en` で出る。
サーバが返す文言（`ServerMessages`）は 67 件すべてが 7 言語ぶん揃っている。
...
1. URL の `?lang=ja` / `?lang=en` / `?lang=zh` など 7 言語のコード（**共有される URL に載る。これが一番強い**）
...
| 場所 | 置き場所 | 鍵の抜けの見つけ方 |
|---|---|---|
| 回答画面 | `Frontend/src/lib/i18n/messages/`（言語ごとの `ja.ts` など） | 型で落ちる（`svelte-check`） |
| 管理画面 | `Frontend/src/admin/lib/i18n/messages/`（言語ごとの `ja.ts` など） | 型で落ちる（`svelte-check`） |
...
`zh`・`de`・`ko`・`es`・`vi` のカタログは `Partial<Record<MessageKey, string>>` なので、
訳し忘れは型検査では落ちず、`en` へ落ちて表示される。

サーバ側は `ServerMessageKeys` の定数とカタログの鍵が
**厳密に一致していること**を単体テストで見る。
定数を足してカタログへ入れ忘れても、その逆でも落ちる。

**文言は 67 件**（`Web/Localization/ServerMessageKeys.cs` の定数の数、2026-09-25 時点）。
各鍵は `Add(鍵, ja, en, vi)` で書き、`vi` だけは `Add` の 3 番目の訳として渡す。
`de`・`es`・`ko`・`zh` は言語ごとの辞書（`german`・`spanish`・`Korean`・`zh`）に分けて持ち、
`Add` の中で `byOtherLanguage` から鍵ごとに拾って `AddAll`（言語ごとの辞書で渡す口）へまとめる。
辞書に訳の無い鍵は入れないので、その言語では `en` へ落ちる（`Web/Localization/ServerMessages.cs`）。
...
- 日付時刻: `Intl.DateTimeFormat`（`ja` → `ja-JP`、`en` → `en-US` など。対応は `language.ts` の `LOCALES`）
...
- **`ServerMessages` の文言は、この確認の時点（46 件）では 2 言語ぶんの書き方で書かれていた。**
  3 言語目以降は `AddAll`（言語ごとの辞書で渡す口）で足せるようにしてあり、
  現在の書き方は 4 章のとおり
- **言語を選ぶ UI は決め打ちになっていない。** `SUPPORTED_LANGUAGES` を回しているだけ
- **書式は `LOCALES` へ 1 行足すだけ**（`zh` → `zh-CN` など）

### ⚠️ 同梱している書体では中国語の字形を揃えられない

**実測した**（2026-09-14）。同梱の Noto Sans JP の `unicode-range` に、
**簡体字にしか無い字（`U+7B80` 简・`U+8FB9` 边・`U+8FD9` 这）が入っていない**
（日本語にもある `U+65E5` 日 は入っている）。

**中国語のカタログは Issue #452 で入ったが、書体は足していない**（2026-09-25 時点）。
同梱の書体は Noto Sans JP などの日本語書体だけ
（`Frontend/src/main.ts`・`Frontend/src/admin/main.ts` の `@fontsource` の読み込み）で、
**簡体字にしか無い字は回答者の端末が持っている書体で表示され、端末によって見え方が変わる。**
揃えたくなったら書体を足す。書体は **SIL OFL 1.1 まで可**（2026-09-08 決定。[`../NOTICE`](../NOTICE)）。
...
Issue #432 以降、**アンケートの中身**に限り、訳が無いときの落とし先を
`SurveyDefinition.FallbackLanguage` でアンケートごとに指定する。
1 章の `LocalizedText.DefaultLanguage`（`ja`）はデータ全体の既定として維持し、
`FallbackLanguage` の既定も `ja` とするため、既存アンケートの表示は変わらない。

- 選べる値は `Core/Localization/SupportedLanguages.cs` の `All` に含まれる言語コードだけ
- 設問・選択肢・見出し・説明・完了画面・自動返信メールは、すべて同じ落とし先を使う
- 公開時は `FallbackLanguage` を `SurveyVersions.DefinitionJson` に含め、公開済みの版を
  後の設定変更から切り離す
- 「編集する言語」は入力先、「訳がないときに表示する言語」は回答者へ出す落とし先であり、
  別の設定として扱う
- 画面自身の文言カタログの落とし先には適用しない

実装の根拠は
`Core/Definitions/SurveyDefinition.cs`、`Core/Definitions/LocalizedText.cs`、
`Data/SurveyDraftStore.cs`、`Data/SurveySnapshotStore.cs`、
`Frontend/src/lib/types.ts`、`Mail/AutoReplyComposer.cs` とする。
