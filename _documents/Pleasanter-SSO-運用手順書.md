...
- **既定の方式（標準の API）では Pleasanter 側に何も置かない。** 定義ファイルも再起動も要らず、
  DBMS ごとの SQL も要らない。拡張 SQL の方式を選んだときだけ、定義ファイルを 1 本置く（[5.3](#53-選べる方式-拡張-sql)）
...
| 本人の確かめ方 | **標準の API（既定）**: `/api/users/get` を 1 回、読むだけ。**拡張 SQL**: 選べる方式として残す（[5 章](#5-pleasanter-側の準備)） |
...
**既定（標準の API）** では、転送した cookie で次の 1 回だけを呼ぶ。**読むだけで、Pleasanter へは何も書かない。**
...
| 事実 | 根拠 |
|---|---|
| API は cookie でも認証される（API キーが無ければ `User.Identity.Name` で利用者を引く） | `Implem.Pleasanter/Libraries/Requests/Context.cs:489-508`、`Implem.Pleasanter/Controllers/Api/ExtendedController.cs:18-36` |
...
| （拡張 SQL）利用者 ID が自動で束縛される（`T`・`D`・`U`） | `Implem.Libraries/DataSources/SqlServer/SqlIo.cs:41-53` |
| （拡張 SQL）束縛の接頭辞は SQL Server が `@_`、それ以外が `@ip` | `Implem.ParameterAccessor/Parts/Parameter.cs:12-16` |
...
**拡張 SQL（2026-09-24。2026-09-25 にも一般利用者で本人が返ることを確認）:**

| 手順 | 結果 |
|---|---|
| cookie なしで拡張 SQL | 401 `{"Id":0,"StatusCode":401,"Message":"Unable to authorize."}` |
| ログイン後の cookie を転送 | 200 `{"StatusCode":200,"Response":{"Data":{"Table":[{"TenantId":1,"UserId":1,"LoginId":"Administrator","Name":"Administrator"}]}}}` |
| `Params` に `_U=2` を混ぜて偽装 | 無視され本人（UserId=1）が返る |
| ログアウト後の（古い）cookie | 401 |
| 登録していない拡張 SQL の名前 | **302**（転送。本アプリは接続不可として扱う） |
| **TOTP 有効**・パスワードだけ通した段階 | 401（`Pleasanter_SessionGuid` しか無い） |
| **TOTP 有効**・コード入力後 | 200 |
...
| API キーが無い | **接続不可**（`http-403`）。ログに設定を案内する警告を出す。API キーを設定するか、拡張 SQL の方式を選ぶ |
...
- **API キーの持ち主と同じテナントの利用者しか引けない**（`users/get` がテナントで絞る）。
  別テナントの利用者は `user-no-row` になる。複数テナントで使うなら拡張 SQL の方式を選ぶ
...
- ⚠️ **成功と判断するのは、JSON の業務ステータスが 200 で、TenantId・UserId（1 以上の整数）・LoginId が
  きっちり読めた 1 行だけ**（標準の API は `TotalCount` も 1）。応答を正しく読めたことを確かめるため。
...
### 5.1 標準の API（既定）: 準備は要らない

**Pleasanter 側に何も置かない。** 定義ファイルも Pleasanter の再起動も要らず、DBMS の違いも関係しない。
本アプリの設定で方式を `StandardApi`（既定）のままにし、内部 URL などを入れるだけでよい（7 章）。
...
`User.json` の `DisableApi` などで利用者の API 利用を禁止しているなら、次のどちらかにする（2.3）。

- **本アプリの Pleasanter 接続設定（`QUESTIONNAIRE_PLEASANTER_BASEURL`・`QUESTIONNAIRE_PLEASANTER_APIKEY`）を設定する。**
  回答の送信で既に設定していれば、追加の作業は無い。キーの持ち主には API の利用を許しておく
  （`DisableApi` のときは利用者の `AllowApi`）
- **拡張 SQL の方式を選ぶ**（5.3）。API キーを使わずに済む

### 5.3 選べる方式: 拡張 SQL

本アプリの設定で方式を `ExtendedSql` にしたときだけ使う。**Pleasanter 側に定義ファイルを置いて再起動する必要がある**
代わりに、API キーを使わず、利用者の API 利用の禁止にも左右されない。

Pleasanter の `App_Data/Parameters/ExtendedSqls/` に、本人 1 行を返す定義を 1 本置く（ファイル名は任意）。
**置いたら Pleasanter を再起動する**（パラメータは起動時に読まれる）。

- `Api: true` が要る（拡張 SQL API から呼べるようにする）
- `Name` は本アプリの設定「拡張 SQL の名前」（既定 `QuestionnaireWhoAmI`）と揃える
- **利用者 ID は必ず自動束縛の変数で絞ること。** 本文の `Params` で渡された値を使う定義にしないこと
  （偽装できてしまう）
- 返す列は `TenantId`・`UserId`・`LoginId`（`Name` は任意）。**別の列で突き合わせたいときは、
  その列を `LoginId` という名前で返す**（例: メールアドレスを本アプリのログイン ID にしている場合）

#### 5.3.1 SQL Server（実機で確認済み）

```json
{
  "Name": "QuestionnaireWhoAmI",
  "Api": true,
  "CommandText": "SELECT \"Users\".\"TenantId\", \"Users\".\"UserId\", \"Users\".\"LoginId\", \"Users\".\"Name\" FROM \"Users\" WHERE \"Users\".\"UserId\" = @_U"
}
```

#### 5.3.2 PostgreSQL・MySQL（**未検証**）

**自動束縛の接頭辞が `@ip` になる**（`Parameter.cs:12-16`）ので `@_U` ではなく `@ipU` と書く。
Pleasanter の `Parameter.json` で `SqlParameterPrefix` を明示している場合はその値に従う。

PostgreSQL（二重引用符で識別子を括る）:

```json
{
  "Name": "QuestionnaireWhoAmI",
  "Api": true,
  "CommandText": "SELECT \"Users\".\"TenantId\", \"Users\".\"UserId\", \"Users\".\"LoginId\", \"Users\".\"Name\" FROM \"Users\" WHERE \"Users\".\"UserId\" = @ipU"
}
```

MySQL（バッククォートで識別子を括る）:
...
> ⚠️ **PostgreSQL・MySQL では実機で確かめていない。** 本番へ入れる前に、5.3.3 の手順で
> 本人が返ることを確かめること。

#### 5.3.3 置いた定義を確かめる

Pleasanter にブラウザでログインしてから、本アプリの **Pleasanter ログイン設定 → 接続の試験** を押す
（保存前の値で問い合わせる）。本人のログイン ID・利用者 ID・テナント ID が出れば定義は正しい。
...
# API キー無しで拡張 SQL を呼ぶ
curl -s -b jar.txt -H "Content-Type: application/json" \
  -d '{"ApiVersion":1.1,"Name":"QuestionnaireWhoAmI"}' http://localhost:8080/api/extended/sql
```
...
`Pleasanter_SessionGuid` しか無く、標準の API（`users/get`・`sessions/set`）も拡張 SQL も 401 になる。
どちらも同じ「認証されているか」の判定（`context.Authenticated`）を通るため。**本アプリは何もしなくても、
Pleasanter の 2 要素を終えた人しか通さない**（2.2 の表。拡張 SQL で TOTP・メールのワンタイムパスワード、
標準の API で TOTP を確認済み。標準の API でのメールのワンタイムパスワードは、同じ判定を通ることからの推論）。
...
| `QUESTIONNAIRE_PLEASANTERSSO_METHOD` | 本人の確かめ方（`StandardApi` / `ExtendedSql`） | `StandardApi` |
| `QUESTIONNAIRE_PLEASANTERSSO_SQLNAME` | 拡張 SQL の名前（`ExtendedSql` のときだけ使う） | `QuestionnaireWhoAmI` |
...
- **標準の API で 403 が返ったときは、本アプリの Pleasanter 接続設定（`QUESTIONNAIRE_PLEASANTER_BASEURL`・
...
3. 内部 URL・ログイン画面・（必要なら）ログアウト画面を入力する。本人の確かめ方は既定の **標準の API** のままでよい
   （拡張 SQL を選んだときだけ、拡張 SQL の名前を入力できる）
...
| 「Pleasanter に問い合わせできませんでした」 | `redirect` | 内部 URL が Pleasanter 以外（ログイン画面など）を指している。拡張 SQL なら名前の違いも（**名前が無いと Pleasanter は 302 を返す**） |
| 同上（標準の API） | `http-403` | 利用者の API 利用が禁止されていて、本アプリに API キーが無い（2.3）。API キーを設定するか拡張 SQL を選ぶ。IP 制限（4.3）・`TokenCheck: true`（4.2）でも起きる |
| 同上 | `http-400` / `http-403` / `http-404` | IP 制限で拒否（4.3）、`TokenCheck: true`（4.2）、内部 URL のパス違い。拡張 SQL なら定義に `Api: true` が無い |
...
| 同上（標準の API） | `no-row` / `multiple-rows` / `no-total-count` / `invalid-row` / `no-data` | 応答が本人 1 行として読めない。内部 URL が Pleasanter の API を指しているかを確認する |
| 同上（拡張 SQL） | `no-row` / `multiple-rows` / `invalid-row` / `no-data` | 拡張 SQL の定義の誤り。利用者 ID を自動束縛の変数（`@_U` / `@ipU`）で絞っているか、列名が `TenantId`・`UserId`・`LoginId` か |
...
| 項目 | 状況 |
|---|---|
| 検証サービスの判定（401・200・業務ステータス 401・転送・HTML・時間切れ・壊れた JSON・UserId 0・cookie の選別） | 単体テスト（`PleasanterSessionVerifierTests`） |
| 標準の API（本文の `Own`・1 行ちょうどと `TotalCount`・403 で API キーの有無による分岐・代わりの経路の 401 と利用者 ID 違い・API キーをログにも cookie 付きの要求にも出さないこと・時間切れが全体に掛かること） | 単体テスト（`PleasanterSessionVerifierStandardApiTests`） |
...
| SQL Server の Pleasanter 実機との往復・再検証・本アプリの 2 要素 | 2026-09-24 に手元で確認（2.2） |
| 標準の API（一般利用者・管理者・未ログイン・`DisableApi` で API キーあり／なし／違うキー） | 2026-09-25 に実機で確認（2.2） |
| Pleasanter の TOTP・メールのワンタイムパスワード | 拡張 SQL は 2026-09-24、標準の API の TOTP は 2026-09-25 に実機で確認（2.2）。**標準の API のメールのワンタイムパスワードは未確認**（同じ判定を通ることからの推論） |
| 利用者ごとの API 禁止（`DisableApi` 以外の禁止） | **未検証**（`UserSettings.AllowApi` の同じ判定に入る） |
| 複数テナント | **未検証**（検証環境のテナントは 1 つ） |
| PostgreSQL・MySQL の Pleasanter | 標準の API は DBMS に依らない（SQL を書かない）が実機では**未検証**。拡張 SQL は**未検証**（5.3.2） |
