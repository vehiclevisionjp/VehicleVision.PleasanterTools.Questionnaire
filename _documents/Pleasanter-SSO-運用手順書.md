...
- **Pleasanter 本体のコードは参照も取り込みもしていない**（規約 6）。Pleasanter 側に置くのは
  拡張 SQL の定義ファイル 1 本だけ

> ⚠️ **本アプリと Pleasanter を同じホスト名で動かすことが前提**（[4 章](#4-前提)）。
> そのために**本アプリをサブパス（例 `/questionnaire/`）で動かす対応は #465 で進めている。**
> #465 が入るまでは、本番の配置（[8 章](#8-配置例)）はそのままでは動かない。
> 手元の検証（`localhost` のポート違い）では動く。
...
sequenceDiagram
    ...
    Q->>P: POST /api/extended/sql（cookie だけ転送。API キーは送らない）
    ...
    loop 既定 5 分ごと（管理 API の要求のたび）
        Q->>P: POST /api/extended/sql（同じく cookie だけ）
...
| 拡張 SQL へ利用者 ID が自動で束縛される（`T`・`D`・`U`） | `Implem.Libraries/DataSources/SqlServer/SqlIo.cs:41-53` |
| 束縛の接頭辞は SQL Server が `@_`、それ以外が `@ip` | `Implem.ParameterAccessor/Parts/Parameter.cs:12-16` |
...
### 2.2 実機での確認（2026-09-24、`tools/pleasanter-testenv`、Pleasanter 1.5.8.1 ＋ SQL Server 2025）
...
- ⚠️ **Pleasanter の API キーを使わない・ブラウザへ渡さない**（規約 8）。転送するのは cookie だけ
...
- ⚠️ **成功と判断するのは、JSON の業務ステータスが 200 で、TenantId・UserId（1 以上の整数）・LoginId が
  きっちり読めた 1 行だけ。** 転送（3xx）・HTML・時間切れ・JSON でないもの・5xx・0 行・複数行は
  すべて「接続不可」で、**決して成功にしない。** 401（HTTP でも業務ステータスでも）だけが「未ログイン」
...
- **cookie の値はログへ出さない。** 転送した個数と応答の状態・時間だけを残す
...
> ⚠️ **本アプリのサブパス対応は #465 で進めている。** それまでは本アプリをサブパスで配信できない。
...
> ⚠️ **PostgreSQL・MySQL では実機で確かめていない。** 本番へ入れる前に、5.3 の手順で
> 本人が返ることを確かめること。
...
`Pleasanter_SessionGuid` しか無く、拡張 SQL は 401 になる。**本アプリは何もしなくても、
Pleasanter の 2 要素を終えた人しか通さない**（2.2 の表で TOTP・メールのワンタイムパスワードとも確認済み）。
...
| `QUESTIONNAIRE_PLEASANTERSSO_SQLNAME` | 拡張 SQL の名前 | `QuestionnaireWhoAmI` |
...
⚠️ **本アプリ側のサブパス対応（`/questionnaire/` での配信・cookie の Path・画面の呼び先）は #465 で進めている。**
以下は #465 が入った後の姿を示す。プレフィックスは剥がさずに本アプリへ渡す想定。
...
次のいずれかで同じホスト名にまとめる（#465 の検討と同じ。出典は参照 2026-09-24）。
...
| 「Pleasanter に問い合わせできませんでした」 | `redirect` | 拡張 SQL の名前が違う（**名前が無いと Pleasanter は 302 を返す**）、または内部 URL が Pleasanter 以外（ログイン画面など）を指している |
| 同上 | `http-400` / `http-403` / `http-404` | 定義に `Api: true` が無い、IP 制限で拒否（4.3）、`TokenCheck: true`（4.2）、内部 URL のパス違い |
...
| 同上 | `no-row` / `multiple-rows` / `invalid-row` / `no-data` | 拡張 SQL の定義の誤り。利用者 ID を自動束縛の変数（`@_U` / `@ipU`）で絞っているか、列名が `TenantId`・`UserId`・`LoginId` か |
...
| Pleasanter の TOTP・メールのワンタイムパスワード | 2026-09-24 に実機で確認（2.2） |
| PostgreSQL・MySQL の Pleasanter | **未検証**（5.2） |
| ブラウザでの画面操作（別窓・自動確認・ログアウトの印） | **未検証** |
| サブパス配置（8 章） | **#465 の後に確認する** |
