...
#   もしくは Pending migrations (SqlServer): に続けて未適用の一覧
...
6. Web App を再起動し、`/healthz` が成功するまで再試行する
...
`PUT /api/zip/site/wwwroot/` で配置し直し、Web App を再起動して `/healthz` を確認する。
...
DB バックアップも復元する必要がある。控え ZIP には `App_Data\Parameters\*.local.json` などの
