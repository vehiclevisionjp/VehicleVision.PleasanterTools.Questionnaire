...
### 3.2.3 アンケート資産の保存先（任意。Issue #271）
...
**Windows Web App を選ぶと ClamAV の App Service サイドカーは使えない。**
...
`/openapi/v1.json` で OpenAPI 3.1 の JSON 文書を出す。人が読む画面は同梱しないため、
Scalar、Swagger UI、Redoc など各自の道具へこの URL を渡す。
...
Azure App Service の更新スクリプトは、実行場所に応じて次の 2 本を使い分ける。

| 実行場所 | スクリプト | 用途 |
| --- | --- | --- |
| `az` を使える運用端末または CI | `scripts\Upgrade-AzureWebApp.ps1` | Azure CLI と Kudu API を使う Zip Deploy |
| 対象 Web App の Kudu | `scripts\Upgrade-InKudu.ps1` | `app_offline.htm` で停止し、共有 content share の `wwwroot` を直接入れ替える |
...
**`%HOME%` はアプリ専用の content share で、再起動後も残り、全インスタンスから共有される。**
...
| 動作中の版を確認したい | ログイン済みの管理画面の右下に表示される「動作中の版」を確認する |
