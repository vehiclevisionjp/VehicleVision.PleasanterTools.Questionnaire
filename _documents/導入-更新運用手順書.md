...
本アプリは既定では独立したホスト名の `/` で動く。**Pleasanter のログインで管理画面へ入る機能を使う場合は、
Pleasanter と同じホスト名のサブパス（例 `/questionnaire/`）に置く**（3.2.7・3.2.8）。
...
マイグレーションは FluentMigrator による**前方適用のみ**である。**起動時の自動適用は v0.5.0 から既定で有効**で、
未適用があれば DB 排他を取って適用し、終わるまでは `/healthz`・`/ready` 以外へ通さない（`/ready` は 503）。`QUESTIONNAIRE_DB_AUTO_MIGRATE=false` で
自動適用を無効にした場合は、未適用のマイグレーションが見つかると安全のため起動を中止する（3.3 の手動コマンドで当てる）
（[`DatabaseStartupMigration.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/DatabaseStartupMigration.cs#L19-L90)、
[`MigrationCommand.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/MigrationCommand.cs#L5-L100)、
[`Program.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L1104-L1113)）。
...
実装の根拠は
[`Program.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L848-L912)、
...
v0.7.0 から、`/admin.html` を直接開く URL は 404 を返す。管理画面の入口はこの設定の値だけである
（[`Program.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L1074)）。
...
Apache・Nginx の例は 2026-09-25 に実機で確かめた。IIS と Azure の置き方は未検証である（同書 6.1）。

⚠️ **サブパスへ移した直後は、以前 `/` で発行された cookie がブラウザに残る。** 管理者は一度ログインし直す。

### 3.2.8 Pleasanter のログインで管理画面へ入る（任意。Issue #464）

Pleasanter にログインしている人が、そのまま管理画面へ入れるようにできる（シングルサインオン）。
**既定は無効。** Pleasanter 側に準備は要らない（標準の API `/api/users/get` で本人を確かめる）。

- ⚠️ **3.2.7 のサブパス配置が前提。** 本アプリと Pleasanter を同じホスト名で動かし、Pleasanter は `/` のままにする
- 設定は `QUESTIONNAIRE_PLEASANTERSSO_*` の外部設定か、管理画面の「Pleasanter ログイン設定」で行う。
  外部設定がある項目は画面でロックされる
- 設定の保存先としてマイグレーション 32（`M0032_PleasanterSsoSettings`）が要る。自動適用なら起動時に当たる

設定の鍵・手順・配置例・トラブルシューティングは
[`Pleasanter-SSO-運用手順書.md`](Pleasanter-SSO-運用手順書.md) を参照する。
...
Pleasanter のログインで管理画面へ入る機能（3.2.8）を使う場合は、Pleasanter と同じホスト名へまとめる必要がある。
別々の `*.azurewebsites.net` に並べる構成では cookie が届かない。Windows の仮想アプリケーション、
または Front Door・Application Gateway のパス振り分けを使う
（[`サブパス配置-運用手順書.md`](サブパス配置-運用手順書.md) 3.4・3.5。Azure での置き方は未検証）。
...
Pleasanter と同じサイトの下へサブアプリケーション（例 `/questionnaire`）として置く場合は、
専用サイトの代わりに [`サブパス配置-運用手順書.md`](サブパス配置-運用手順書.md) 3.1 に従う
（アプリケーションプールは Pleasanter と分ける。IIS での置き方は未検証）。
...
| `/admin.html` を開くと 404 | 仕様（v0.7.0 から）。管理画面は `QUESTIONNAIRE_ADMIN_PATH`（既定 `/admin`）から開く |
| Pleasanter のログインで入れない | [`Pleasanter-SSO-運用手順書.md`](Pleasanter-SSO-運用手順書.md) 9 章 |
...
- `Program.cs` の[必須設定](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L95-L147)、
  起動時のマイグレーションと readiness 制御、
  [HTTPS](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L928-L937)と
  [`/healthz`・`/ready`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/Program.cs#L1076-L1103)
- [`MigrationCommand.cs`](../src/VehicleVision.PleasanterTools.Questionnaire.Web/MigrationCommand.cs#L5-L100)
