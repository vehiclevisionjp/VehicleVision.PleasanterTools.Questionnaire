...
| `QUESTIONNAIRE_DB_AUTO_MIGRATE` | `true` のときだけ起動時に未適用マイグレーションを適用する。既定は無効 |
...
`QUESTIONNAIRE_DB_AUTO_MIGRATE=true` を使う場合、Web アプリの DB アカウントには
スキーマ変更権限も必要になる。最小権限を優先して実行用アカウントと分ける場合は、
自動適用を有効にせず 3.3 の手動コマンドを使う。
...
> 返信先と BCC はこの制約を受けない。
>
> - 出典: [Add multiple sender addresses to Email Communication Service](https://learn.microsoft.com/azure/communication-services/quickstarts/email/add-multiple-senders#create-multiple-sender-usernames)（参照日 2026-09-17）
> - 出典: [Dynamic Display Name in MailFrom addresses（Microsoft Q&A）](https://learn.microsoft.com/answers/a/1863125)（参照日 2026-09-17）
...
起動時の自動適用を使う場合は `QUESTIONNAIRE_DB_AUTO_MIGRATE=true` を設定する。
既定は無効であり、未設定のまま未適用があると従来どおり起動を中止する。
