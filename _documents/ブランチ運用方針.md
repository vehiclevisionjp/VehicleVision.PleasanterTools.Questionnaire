...
git worktree add -b feature/189-auto-reply-mail \
    ../VehicleVision.PleasanterTools.Questionnaire.worktrees/189-auto-reply-mail origin/develop
gh pr create --base develop

# 子: 親のブランチから分岐し、base も親のブランチにする
git worktree add -b feature/202-response-edit-link \
    ../VehicleVision.PleasanterTools.Questionnaire.worktrees/202-response-edit-link \
...
- **base の付け替えが自動で起きることを前提にしない。**
  ...
  **中身の取り込みまではしない**
- **マイグレーション番号が重なっていないかを、ここで必ず見直すこと。**
  ⚠️ **FluentMigrator は番号が重なると黙って飛ばす**（下記「マイグレーションの番号」）
...
**並行して走る PR が、それぞれマイグレーションを足すと番号がぶつかる。**
⚠️ **FluentMigrator は同じ番号を 2 つ見つけても落ちず、片方を黙って飛ばす。**
**当たっていないのに「すべて当たっている」と言われる**ので、気付くのが遅れる。

- **Issue を投げる時点で番号を割り当てること**
- **先にマージされる側が若い番号を取り、後から入る側をずらす**
- 衝突に気付いたら、検証環境の DB は `VersionInfo` の行を消してから当て直す
...
| `.github/workflows/integration.yml` | `4 RDBMS 結合テスト` | 根幹ブランチへの push、根幹ブランチを対象とする PR | ○ |
| `.github/workflows/e2e.yml` | `端から端まで通す試験` | 根幹ブランチへの push、**`develop` を対象とする PR のうちコードを触るもの**、日次（03:00 JST）、手動。**PR・push・手動は SQL Server、日次は PostgreSQL と MySQL** | ○（`develop` のみ） |
| 同上 | `端から端まで通す試験（{RDBMS}）` / `写しの一式` | 同上。前者は RDBMS ごとの本体で、結果は `端から端まで通す試験` がまとめて報告する | — |
| 同上 | `失敗を Issue で知らせる` / `直ったことを Issue へ知らせる` | 同上のうち根幹ブランチへの push と日次だけ | — |
| `.github/workflows/e2e-skip.yml` | `端から端まで通す試験`（同じ名前で報告するだけ） | **`develop` を対象とする PR のうちコードを触らないもの** | ○（`develop` のみ） |
| `.github/workflows/pr-title-wip-check.yml` | `WIP が外れているか` | 根幹ブランチを対象とする PR（タイトルの編集も拾う） | ○ |
| `.github/workflows/pr-issue-link-check.yml` | `Issue と紐付いているか` | 根幹ブランチを対象とする PR（本文の編集も拾う）。止めずにコメントで知らせる | — |
| `.github/workflows/cla-check.yml` | `CLA へ署名しているか` | 根幹ブランチを対象とする PR（`pull_request_target`） | — |
| `.github/workflows/dependency-review.yml` | `入ってくる依存を見る` | 根幹ブランチを対象とする PR | — |
| `.github/workflows/aks-image.yml` | `ACRへ発行` | 手動のみ | — |
...
| `.github/workflows/release-run.yml` | `版を採番して前提を確かめる` / `リリースを通す` | 手動のみ（`develop` から） | — |
...
- **端から端まで通す試験は、コードを触る PR でも回す**（Issue #399）。
  ⚠️ **メールが実際に届くかを確かめているのはここだけ**で、
  #397 は PR の CI が緑のままマージされ、develop へ入ってから分かった。
  ドキュメントだけの PR では回らない（`paths` で絞ってある）
    - ⚠️ **必須チェックを `paths` で絞ると、条件に当たらない PR では永久に Pending になる**
      （GitHub は起動しなかったワークフローのチェックを未完了のまま残す）。
      **同じジョブ名を反対の条件で成功させる `e2e-skip.yml` を置いてある。**
      ⚠️ **`paths` を変えたら、あちらの `paths-ignore` も同じ内容へ揃えること**
    - **以前は「PR ごとには回さない」としていた**（Pleasanter 実機の起動に数分かかるため）。
      ⚠️ **その根拠は実測で崩れた。** 丸ごとで 7〜8 分、Pleasanter を使わないクラスだけに
      絞ってもテスト本体で 6 分かかる。**重いのは起動ではなく試験本体**
      （1 件ごとに管理者を作り直す）。**絞ると PR で回す一覧の保守が要るので丸ごと回す**
    - **Pleasanter 本体（AGPL v3）は公式イメージを起動するだけ。**
      ソースの参照・コピー・リンク・プロジェクト参照はしない。
      起動しているのは「相手方の実機」であって、本製品の一部ではない
    - **全 E2E クラスを同じ RDBMS へ当てる。** PR・push は SQL Server だけにして
      起動するコンテナを減らし、日次は PostgreSQL と MySQL を一巡して互換性を確かめる
    - HTTP 系は起動中の app が使う `Questionnaire`（PostgreSQL / MySQL は
      `questionnaire`）を直接読み、in-process 系は別の `Questionnaire_E2E`
      （PostgreSQL / MySQL は `questionnaire_e2e`）を使う。**同じ DB にすると、
      並行する試験が互いの行を消す**
...
- **このワークフローでは `actions/dependency-review-action` を使わない。** GitHub の依存関係グラフを前提にする
  ...
  上記 2 つは GitHub 側の有効化状態に関係なく動く
    - PR で**これから入る**依存（脆弱性とコピーレフトのライセンス）は、別の `dependency-review.yml` が
      この action で見ている（必須にはしていない）。2026-09-25 時点で直近の実行は成功している
...
> `EndToEndTests` / `AdminAuthEndToEndTests` / `PleasanterApiClientTests` など Pleasanter 本体の検証環境が要る試験は、
> 以前はここに挙げていたが、今は `e2e.yml` が回している
> （`--filter "FullyQualifiedName~EndToEndTests|FullyQualifiedName~PleasanterApiClientTests"`。
> Pleasanter のログインの試験も `compose.pleasanter-sso.yaml` を重ねて同じジョブで回す）。
> **`integration.yml` と単体テストでは回らない**ので、それらが緑でもこの範囲は検証されていない。
...
- **リポジトリ管理者（Repository role の Admin）はバイパスできる。** 緊急時の復旧経路を塞がないため。
  **通常運用でバイパスを使わないこと**
    - 2026-09-25 に `GET /repos/{owner}/{repo}/rulesets/{id}` で確かめた時点では、3 つの ruleset とも
      バイパスの対象はこの 1 つだけで、自動化 bot（`my-repo-automation-bot`）は入っていない。
...
（2026-09-25 に ruleset の API で確かめた。`develop-protection` は 7 つ、`master-protection` は 6 つ）。
...
| `フロントエンド` | 同上 |
| `ライセンス検査` | 同上 |
| `4 RDBMS 結合テスト` | `.github/workflows/integration.yml` |
| `WIP が外れているか` | `.github/workflows/pr-title-wip-check.yml` |
| `NOTICE と依存の整合検査` | `.github/workflows/dependency-audit.yml` |
| `端から端まで通す試験` | `.github/workflows/e2e.yml` / `e2e-skip.yml`（**`develop-protection` だけ**） |
...
**端から端まで通す試験は `develop` でだけ必須にしている。** Issue #399 以降、`develop` を対象とする
コードを触る PR でも回すようにし、触らない PR は `e2e-skip.yml` が同じ名前で成功を報告する。
`master` 向けの PR（リリースの取り込み）では回らないため、`master-protection` には登録していない。
