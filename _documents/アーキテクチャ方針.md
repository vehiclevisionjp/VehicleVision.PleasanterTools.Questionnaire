...
## 10. Pleasanter へ届かなかった回答の扱い

**Pleasanter が落ちている、または `Upsert` が失敗したときは、回答を本アプリの DB へ
一時保管し、後から再送する**（2026-08-18 決定）。
...
sequenceDiagram
    ...
    participant D as 本アプリのDB<br/>(一時保管)
    participant P as Pleasanter
...
**回答データの正本は Pleasanter**（1 章の線引き）。ここは**送れるまで預かる置き場**であり、
**送信できたら消す。**

- **一時保管を集計・エクスポートの対象にしないこと。** 参照先を増やすと、
  「回答の正本はどこか」が曖昧になる
- 管理画面には**未送信件数と最古の滞留時刻**を出す。溜まっていることに気づけないのが一番まずい

### GUID ＋ `Upsert` が効く

**再送が冪等になる。** 回答ごとの GUID をキーに `Upsert` する設計（9 章）なので、
**送信できたか判断がつかないまま再送しても、レコードは 1 件にしかならない。**

- 「送ったが応答が返らなかった」場合も、**そのまま再送してよい**
- 再送ワーカーが多重に起動しても、二重登録にはならない

### 個人情報が自前 DB を経由する

**回答本文が一時的に本アプリの DB に載る。** 氏名・メール・自由記述が含まれ得る。
...
- **スケールアウトすると同じ回答を複数インスタンスが拾う。**
  冪等なので実害は小さいが、**取り出しの排他は入れる**
- **スロット swap と再起動で処理が中断される。** 中断されても失われない作りにする
  （取り出し中に落ちたら未送信へ戻る）
- 常駐処理を動かすなら **Always On** が要る。無効だとアイドルで停止して再送が止まる
...
回答に添付があると、**バイナリも一時保管することになる。**
DB に入れるか、別のストレージへ置くかは未確定（「未確定事項」参照）。

### 未確定：常に経由するか、失敗時だけ退避するか

| 方式 | 利点 | 欠点 |
|---|---|---|
| **常に一時保管を経由** | 経路が 1 本。退避と再送が毎回動くので腐らない。ピークを吸収できる | 常に個人情報が自前 DB を経由する。Pleasanter への反映が常に遅延する |
| **失敗時だけ退避** | 正常系が単純で遅延が無い | 経路が 2 本。**稀にしか通らない経路は本番で初めて動いて壊れる** |
...
| 6 | 管理者の棚卸し手順（退職者の止め忘れを運用でどう防ぐか） | 要件確認 |
| 7 | **一時保管を常に経由するか、失敗時だけ退避するか**（10 章） | 次に決める |
| 8 | 一時保管の滞留上限と、超えたときの扱い。デッドレターの通知先 | 要件確認 |
| 9 | 添付ファイルの投稿経路（`api/binaries` 系の利用可否）と、一時保管でのバイナリの持ち方 | `_reference` 調査 ＋ 実機 |
| 10 | 「どのサイトを回答フォームとして公開してよいか」の指定方法（許可リストか、サイト側の目印か） | 要件確認 |
| 11 | DB スキーマのマイグレーション手段（3 RDBMS で同じ手順にできるか） | 実装時に決める |
| 12 | 回答者への自動返信メールの要否（Pleasanter の通知機能を使うか本アプリで送るか） | 要件確認 |
...
| 送信失敗時の回答 | **本アプリの DB へ一時保管して再送する。** 一時保管は蓄積先ではなく、送信できたら消す |
