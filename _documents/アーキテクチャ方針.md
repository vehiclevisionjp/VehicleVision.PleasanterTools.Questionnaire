...
| # | 論点 | 確認方法 |
|---|---|---|
| 1 | マッピング編集 UI のノードエディタ部品を使うか自作か。**採用するならライセンスを一次情報で確認** | 一次情報で確認 |
| 2 | 3 RDBMS 対応のマイグレーションツールの選定。**ライセンスを一次情報で確認** | 一次情報で確認 |
| 3 | bot・スパム対策の方式 | 要件確認 |
| 4 | 送信待ちの滞留上限と、デッドレター・滞留超過・401 検知の通知先 | 要件確認 |
| 5 | 監査ログとデッドレターの保持期間 | 要件確認 |
| 6 | 同時回答数と回答上限の想定値 | 要件確認 |
| 7 | 管理者の棚卸し手順（退職者の止め忘れを運用でどう防ぐか） | 要件確認 |
| 8 | 第 1 弾のリリース範囲（[`機能一覧.md`](機能一覧.md) の ◎ のみか、○ まで含めるか） | 要件確認 |
| 9 | [`機能一覧.md`](機能一覧.md) を Forms 両サービスの公式ドキュメントと突き合わせる | 一次情報で確認 |
