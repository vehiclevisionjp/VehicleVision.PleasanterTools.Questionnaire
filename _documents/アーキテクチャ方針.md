...
### 独自ログインの帰結：管理者の資格情報をどこへ置くか

**本アプリは独自 DB を持たない**という方針との間に緊張がある。
独自ログインを持つ以上、**管理者アカウントとパスワードハッシュの置き場所が要る。**

- 管理者アカウントには、**対応する Pleasanter のユーザ（`LoginId` または `UserId`）を
  必ず持たせる。** これが無いとグループ・組織による認可ができない
- パスワードは**ハッシュのみ**を保存する（平文・可逆暗号にしない）
- 置き場所は未確定。「未確定事項」参照

> **注意。** 設定保存用サイトへ置く場合、**そのサイトを Pleasanter 上で閲覧できる人に
> ハッシュが見える。** サイトの権限を絞ること。
...
| 9 | **管理者アカウントとパスワードハッシュの置き場所**（設定保存用サイト / 本アプリの小さな DB / パスワードレス） | 次に決める |
