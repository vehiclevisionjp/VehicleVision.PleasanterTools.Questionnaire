...
回答者を区別したい場合は、次のいずれかを別途設ける。

- 回答者を識別する項目をサイト設定側に用意し、本アプリが値を詰める
- 回答用 URL に一意のトークンを持たせ、本アプリ側で回答者と対応付ける

**どちらを採るかは未確定**（「未確定事項」参照）。
...
Pleasanter と本アプリを Azure App Service で併設して動かす想定。

```mermaid
flowchart TB
    subgraph PLAN["App Service プラン"]
        A["Web アプリ: Pleasanter"]
        B["Web アプリ: 本アンケートアプリ"]
    end
    B -->|"標準 API（APIキー）<br/>サーバ間通信"| A
```

- 本アプリ → Pleasanter は**サーバ間通信**なので、ブラウザ側の CORS は発生しない
- API キー・Pleasanter のベース URL は**アプリケーション設定（環境変数）**で外部化する。
  デプロイスロットごとに別の値を差せるようにする
- **アプリ設定 `WEBSITE_TIME_ZONE` に `Tokyo Standard Time` を設定する**（前章）

> **要確認。** 「スロットを追加して両方動かす」という言い方があったが、App Service の
> **デプロイスロットは同一 Web アプリの複製**であり、別アプリを載せる仕組みではない。
> 別アプリとして動かすなら **同じ App Service プラン上に Web アプリをもう 1 つ作る**構成になる。
> どちらの意図かを確認して本節を確定させる。
...
| 3 | 回答者の識別方法（サイト設定の項目に持たせる／URL トークン） | 要件確認 |
| 4 | 匿名公開の可否と、公開範囲の制御方法 | 要件確認 ＋ 実機 |
| 5 | 添付ファイルの投稿経路（`api/binaries` 系の利用可否） | `_reference` 調査 ＋ 実機 |
| 6 | 1 アンケート＝1 Pleasanter サイトとするか、複数アンケートを 1 サイトに同居させるか | 要件確認 |
| 7 | Azure の構成（デプロイスロットか、同一プラン上の別 Web アプリか） | 要件確認 |
| 8 | 回答の重複防止・再編集の可否 | 要件確認 |
