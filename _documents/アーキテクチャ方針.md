...
- **アンケートの定義・拡張設定・回答は、すべて Pleasanter 側へ保存する**（7 章）。
  **例外は管理者の認証情報のみ**で、これだけは本アプリの小さな DB に持つ（10 章）
...
| 何を | どこへ |
|---|---|
| アンケートの定義・拡張設定 | Pleasanter の設定保存用サイト |
| 回答データ | Pleasanter の回答サイト |
| **管理者の認証情報のみ** | **本アプリの DB** |

**この例外を広げないこと。** アンケートに関わるデータを本アプリの DB へ置き始めると、
Pleasanter をバックエンドにする意味が薄れ、二重管理になる。
...
Pleasanter の設定保存用サイトへ置かない理由は、**そのサイトを Pleasanter 上で
閲覧できる人にハッシュが見えてしまい、サイト権限の設定ミスがそのまま事故になる**ため。
...
| 9 | **管理者認証用 DB を何にするか**（Pleasanter と同じ DB サーバ上の別データベース / Azure SQL / SQLite） | 次に決める |
