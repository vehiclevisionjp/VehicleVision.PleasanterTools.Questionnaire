...
検証部分を通せる。**リリース作業に入る前に一度回すこと。** 既存の `v*` タグが無い
初回 Release 前は、`Directory.Build.props` の `VersionPrefix` を Release 候補として使う。
dry-run ではタグと `master` の包含関係だけを省き、本番のタグ push では必ず検査する。
